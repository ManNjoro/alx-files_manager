...
class FilesController {
  static async postUpload(request, response) {
    ...
    if (!essentials.isValidId(userId)) {
    ...
    if (fileParams.parentId !== 0 && !essentials.isValidId(fileParams.parentId)) { return response.status(400).send({ error: 'Parent not found' }); }
  ...
  static async getShow(request, response) {
    ...
    if (!essentials.isValidId(fileId) || !essentials.isValidId(userId)) { return response.status(404).send({ error: 'Not found' }); }
  ...
  static async getIndex(request, response) {
    ...
    if (parentId !== 0 && parentId !== '0') {
      if (!essentials.isValidId(parentId)) { return response.status(401).send({ error: 'Unauthorized' }); }
